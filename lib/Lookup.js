...
class Lookup {
  ...
  constructor(options) {
    ...
    // just assign them 1 by 1 to make linter happier
    this.childEntity = options.childEntity
    ...
    this.parentFields = options.parentFields
    this.relatedListName = options.relatedListName
    this.relatedListFields = options.relatedListFields
  ...
  registerEvents(sfSync) {
    sfSync.onReceived('inserted', this.parentEntity, this._onParentInserted.bind(this))
    sfSync.onReceived('updated', this.parentEntity, this._onParentUpdated.bind(this))
    if (this.relatedListName) {
      ...
      sfSync.onReceived('removed', this.childEntity, this._onChildRemovedEvent.bind(this))
    }
  }
  ...
  onChildSynced(rec) {
    ...
    } else if (this.lookupField in rec) {
      // make sure that it was indeed retrieved with the record
      if (this.parentFieldName) {
  ...
  _onChildRemovedHook(userId, record) {
    console.log('child removed hook', record);
    if (this.relatedListName && record[this.lookupField]) {
      removeChildFromCollection(record.Id, this.relatedListName, this.parentCollection, record[this.lookupField])
    }
  }

  // Update the parent collection.
  // This runs when a delete is synced from Salesforce
  _onChildRemovedEvent({Id}) {
    if (this.relatedListName) {
      removeChildFromCollection(Id, this.relatedListName, this.parentCollection)
    }
  }
...
function removeChildFromCollection(childId, relatedListName, parentCollection, parentId = '') {
  // parent id may be passed to help the query, but it won't be available in all events
  console.log(`Pull child id ${childId} from ${relatedListName}`);
  const query = parentId ? {Id: parentId} : {[relatedListName]: {$elemMatch: {Id: childId}}}
  parentCollection.update(query, {$pull: {[relatedListName]: {Id: childId}}})
}
