...
describe('Lookup', () => {
  ...
  it('register events', () => {
    mockSfSync.onReceived.should.be.called
  })
  ...
  it('updates the parent collection when a child is removed', () => {
    parentCollection.update({Id: 'theparent'}, {$set: {_children: [{Id: 'thechild', OtherKey: 'something'}]}})
    lookup._onChildRemovedEvent({Id: 'thechild'})
    const parent = parentCollection.findOne({Id: 'theparent'})
    parent._children.should.eql([])
  })

  it('updates the parent collection when a child is removed directly', () => {
    const child = {Id: 'thechild', Name: 'Child 1', Parent__c: 'theparent'}
    childCollection.insert(child)
    parentCollection.update({Id: 'theparent'}, {$set: {_children: [{Id: 'thechild', OtherKey: 'something'}]}})
    childCollection.remove({Id: 'thechild'})
    const parent = parentCollection.findOne({Id: 'theparent'})
    parent._children.should.eql([])
  })
